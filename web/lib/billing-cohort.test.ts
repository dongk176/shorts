...
describe("billing customer cohort", () => {
  ...
  it("reactivates persisted Toss customers that were temporarily routed to legacy checkout", () => {
    const legacyRouted = {
    ...
    expect(shouldUseTossBillingExperience(legacyRouted)).toBe(true);
    expect(requirePersistedTossBillingCustomer(legacyRouted)).toEqual(legacyRouted);
  });
