...
describe("selected source range", () => {
  ...
  it("parses seconds, minute timestamps, and hour timestamps", () => {
    expect(parseSourceTimestampInput("90")).toBe(90);
    expect(parseSourceTimestampInput("1:30")).toBe(90);
    expect(parseSourceTimestampInput("1:02:03.5")).toBe(3723.5);
    expect(parseSourceTimestampInput("1:60")).toBeNull();
    expect(parseSourceTimestampInput("wrong")).toBeNull();
  });
});
