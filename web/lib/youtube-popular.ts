...
export type PopularDiscoveryPeriod = (typeof popularDiscoveryPeriods)[number];
export type PopularReusablePeriod = PopularDiscoveryPeriod;
...
export type PopularVideoResponse = {
  ...
  updatedAt: string;
  totalCount?: number;
  periodCounts?: Record<PopularDiscoveryPeriod, number>;
  /** @deprecated Use periodCounts. Kept for older API consumers. */
...
export async function getPopularVideos(
  ...
  const run = await resolveReadyRun(
    db,
    decodedCursor?.runId,
    reusableOnly || discoveryPeriod !== "all",
  );
    ...
    const rows = await db`
      with run_window as (
        select
          ...
          coalesce((
            ...
            from shorts_mvp.popular_video_runs previous_run
        ...
        from shorts_mvp.popular_video_runs current_run
      ...
      historical_candidates as (
        ...
        where r.status='ready'
          ...
          and (${reusableOnly}=false or i.license='creativeCommon')
          and (${type}<>'views' or ${reusableOnly}=true or i.license <> 'creativeCommon')
      ...
      scoped as (
        ...
        where duplicate_rank=1
          ...
          and (
            ...
            or exists (
              select 1
              from shorts_mvp.popular_video_items current_item
              where current_item.run_id=${run.id}
                and current_item.video_id=historical_candidates.video_id
                and (${type}<>'views' or current_item.license <> 'creativeCommon')
      ...
      paged as (
        select
          ...
          view_count, published_at, license, category_rank, last_seen_at, first_seen_at,
          row_number() over (
            order by
              case when ${discoveryPeriod}<>'all' then first_seen_at end desc,
              case when ${type}='views' then view_count end desc,
              case when ${type}='trending' and ${reusableOnly}=true then last_seen_at end desc,
              case when ${type}='trending' then category_rank end asc,
              last_seen_at desc, published_at desc, video_id asc
          ) as page_rank
        from scoped
        cross join run_window
        where (
      ...
      select
        ...
        paged.published_at, paged.license, paged.category_rank,
        paged.last_seen_at, paged.first_seen_at,
