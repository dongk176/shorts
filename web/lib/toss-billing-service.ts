...
import { assertTossCardIssuerAllowed } from "@/lib/toss-billing-runtime";
import {
...
export async function loadTossChargeCredentials(input: {
  ...
  await assertTossCardIssuerAllowed(db, paymentMethod.issuerCode);
  return {
...
export async function registerTossBillingKey(input: {
  ...
  if (existing) {
    ...
    await assertTossCardIssuerAllowed(db, existing.issuerCode);
    return {
      id: existing.id,
  ...
  try {
    await assertTossCardIssuerAllowed(db, issued.card?.issuerCode);
  } catch (error) {
    try {
      await (input.removeIssuedKey ?? deleteTossBillingKey)(issued.billingKey);
    } catch {
      // The key is not persisted or charged. Provider-side cleanup can be
      // reconciled without exposing the raw billing key.
    }
    throw error;
  }
...
async function runPreparedCharge(input: {
  ...
  await assertTossCardIssuerAllowed(input.db, paymentMethod.issuerCode);
