...
export default function PrivacyPage() {
  return (
    <LegalDocument eyebrow="Privacy Policy" title="개인정보처리방침" description="Easy Cut은 이용자의 개인정보를 필요한 범위에서만 처리하고 안전하게 보호하기 위해 다음과 같이 개인정보처리방침을 공개합니다." effectiveDate="2026년 7월 15일">
      ...
      <LegalSection title="3. 개인정보의 처리 및 보유 기간">
        <ul><li>• 계정 정보: 회원 탈퇴 또는 계정 삭제 요청 시까지</li><li>• 로그인 세션 쿠키: 발급일 또는 마지막 갱신일부터 최대 30일</li><li>• 전체 원본 영상·추출 오디오·전사 중간 데이터: 쇼츠 생성 작업 중 임시 저장하며 작업 종료 시 삭제</li><li>• 편집용 클립·완성 영상·썸네일·자막 구간: 요금제 정책에 따라 최초 생성일부터 최대 30일</li><li>• 프로젝트 및 이용량 기록: 계정 유지 기간 또는 운영·분쟁 대응에 필요한 기간</li></ul>
      ...
      <LegalSection title="5. 처리업무의 위탁 및 외부 서비스 이용">
        ...
        <div className="overflow-x-auto"><table><thead><tr><th>수탁자</th><th>업무 내용</th><th>처리될 수 있는 정보</th></tr></thead><tbody>
          ...
          <tr><td>Google</td><td>소셜 로그인, YouTube 정보 조회 및 AI 구간 분석</td><td>로그인 정보, YouTube URL·메타데이터·전사 텍스트</td></tr>
      ...
      <LegalSection title="6. AI를 이용한 데이터 처리">
        <p>Easy Cut은 모든 작업 영상의 오디오를 OpenAI로 전사합니다. 생성된 전사 텍스트와 영상 메타데이터는 하이라이트 구간 선정 및 제목 생성을 위해 Gemini에 전달될 수 있으며, Gemini가 설정되지 않았거나 요청·응답 처리에 실패하면 OpenAI에 전달될 수 있습니다. 운영팀은 이용자의 콘텐츠를 자체 AI 모델 학습 데이터로 별도 활용하지 않습니다.</p>
