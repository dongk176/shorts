...
export default function PrivacyPage() {
  return (
    <LegalDocument eyebrow="Privacy Policy" title="개인정보처리방침" description="아티룸(이하 “회사”라 한다)은 Easy Cut을 운영하며, 이용자의 개인정보를 필요한 범위에서만 처리하고 안전하게 보호하기 위해 다음과 같이 개인정보처리방침을 공개합니다." effectiveDate="2026년 7월 28일" translations={privacyTranslations} sectionIds={{ 5: "international-transfers" }}>
      <LegalSection title="1. 개인정보의 처리 목적">
        <p>회사는 다음 목적을 위해 개인정보를 처리합니다.</p>
        <ul><li>• Google 등 소셜 로그인, 이용자 식별 및 계정 관리</li><li>• YouTube 영상 분석, 쇼츠 생성·편집·다운로드 및 프로젝트 관리</li><li>• 구독·추가 상품 결제, 정기결제, 결제 취소 및 거래 대사</li><li>• 레퍼럴 링크 유입 확인, 신규 회원의 추천인 자동 귀속, 파트너 수익 및 정산 관리</li><li>• 서비스 이용량 산정, 요금제 한도 적용 및 부정 이용 방지</li><li>• 방문·이용 통계 분석 및 서비스 개선</li><li>• 오류 대응, 보안 유지, 서비스 품질 개선 및 이용자 문의 처리</li></ul>
      </LegalSection>
      <LegalSection title="2. 처리하는 개인정보 항목">
        <div className="overflow-x-auto"><table><thead><tr><th>구분</th><th>처리 항목</th></tr></thead><tbody>
          ...
          <tr><td>자동 생성 정보</td><td>세션 쿠키, 접속 시각, IP 주소, 브라우저·기기 정보, 요청 및 오류 기록, 유료 기능 사용 시각·필터 조건·결과 수와 연결된 구독·주문 식별자, Google Analytics에 쿠키나 지속 방문자 식별자 없이 전송되는 방문 페이지, 유입 경로, 대략적 지역 및 이용 이벤트</td></tr>
        </tbody></table></div>
      ...
      <LegalSection title="5. 개인정보 처리업무의 위탁">
        ...
        <div className="overflow-x-auto"><table><thead><tr><th>수탁자</th><th>위탁 업무</th><th>처리될 수 있는 정보</th><th>주 처리 위치</th></tr></thead><tbody>
          ...
          <tr><td>Google LLC</td><td>Google 로그인, YouTube 정보 조회, Gemini AI 분석 및 쿠키 없는 방문 통계</td><td>로그인 정보, YouTube URL·메타데이터, 전사 텍스트, 쿠키 없는 방문·기기·이용 이벤트</td><td>미국 등 글로벌 인프라</td></tr>
      ...
      <LegalSection id="international-transfers" title="6. 개인정보의 국외 이전">
        <p>회사는 아래와 같이 개인정보를 국외로 이전합니다. 서비스 제공에 필수적인 처리위탁·보관은 「개인정보 보호법」 제28조의8 제1항 제3호에 따라 이용계약의 체결·이행에 필요한 범위에서 수행합니다. Google Analytics는 분석 저장을 항상 거부하는 쿠키 없는 제한 모드로 구성하여 분석 쿠키나 지속 방문자 식별자를 읽거나 쓰지 않고 기본 방문 측정 정보만 전송합니다.</p>
        <div className="overflow-x-auto"><table><thead><tr><th>이전받는 자·연락처</th><th>이전 국가</th><th>이전 항목</th><th>시기·방법</th><th>목적</th><th>보유·이용 기간</th><th>거부 방법·효과</th></tr></thead><tbody>
          ...
          <tr>
            <td>Google LLC (Google Analytics, 쿠키 없는 측정)<br /><a href="https://support.google.com/policies/contact/general_privacy_form" target="_blank" rel="noreferrer">개인정보 문의</a></td>
            <td>미국 및 Google 데이터센터 운영 국가</td>
            <td>접속 시각, 브라우저·기기 정보, 방문 페이지, 유입 경로, 대략적 지역, 동의 상태, 페이지 로드마다 생성되는 임시 난수 및 전송 과정에서 처리될 수 있는 IP 주소</td>
            <td>웹사이트 방문·페이지 전환 시 TLS 암호화 통신으로 수시 이전</td>
            <td>쿠키 없는 기본 방문 측정 및 통계 모델링</td>
            <td>Google Analytics 속성에 설정된 이벤트 보유기간까지. 브라우저에는 분석 쿠키를 저장하지 않음</td>
            <td>브라우저의 추적 방지 또는 콘텐츠 차단 기능으로 전송을 제한할 수 있으며, 일반 서비스 이용에는 영향이 없습니다.</td>
          </tr>
        </tbody></table></div>
      ...
      <LegalSection title="9. 쿠키의 사용"><p>Easy Cut은 로그인 상태와 프로젝트 소유권을 유지하기 위한 필수 세션 쿠키와 레퍼럴 링크의 최초 추천인을 기억하기 위한 1년 만료 쿠키를 사용합니다. 레퍼럴 쿠키에는 임의 토큰만 저장하며 실제 파트너 연결정보는 서버에 토큰 해시와 함께 보관합니다. Google Analytics는 분석 저장을 거부한 상태로 동작하므로 분석 쿠키나 지속 방문자 식별자를 읽거나 쓰지 않습니다. 광고 저장, 광고 사용자 데이터, 광고 개인화 및 Google Signals 기능도 사용하지 않습니다. 필수 쿠키를 차단하면 로그인·프로젝트 또는 추천인 자동 귀속 기능이 정상적으로 동작하지 않을 수 있습니다.</p></LegalSection>
      <LegalSection title="10. 이용자의 권리와 행사 방법"><p>이용자는 자신의 개인정보에 대해 열람, 정정, 삭제, 처리정지, 동의 철회 및 국외이전 거부를 요청할 수 있습니다. 서비스 내 고객 지원을 통해 요청하면 본인 확인 후 관련 법령이 정한 절차에 따라 처리합니다. 필수 처리나 국외이전을 거부하면 해당 정보가 필요한 기능은 제한될 수 있습니다. 쿠키 없는 분석 전송은 브라우저의 추적 방지 또는 콘텐츠 차단 기능으로 제한할 수 있으며 일반 서비스 이용에는 영향이 없습니다.</p></LegalSection>
