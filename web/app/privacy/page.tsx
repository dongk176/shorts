...
export default function PrivacyPage() {
  return (
    <LegalDocument eyebrow="Privacy Policy" title="개인정보처리방침" description="아티룸(이하 “회사”라 한다)은 Easy Cut을 운영하며, 이용자의 개인정보를 필요한 범위에서만 처리하고 안전하게 보호하기 위해 다음과 같이 개인정보처리방침을 공개합니다." effectiveDate="2026년 8월 24일" translations={privacyTranslations} sectionIds={{ 5: "international-transfers" }}>
      ...
      <LegalSection title="2. 처리하는 개인정보 항목">
        <div className="overflow-x-auto"><table><thead><tr><th>구분</th><th>처리 항목</th></tr></thead><tbody>
          ...
          <tr><td>결제</td><td>구매자 이름·이메일·암호화된 연락처, 결제 상품·금액·거래번호, 카드사·카드 종류·마스킹 카드번호, 암호화된 더페이원 카드 ID 또는 토스 빌링키, 토스 고객 식별자</td></tr>
        ...
        <p>Easy Cut은 Google 또는 다른 소셜 로그인 제공자의 비밀번호를 수집하거나 저장하지 않습니다. 카드번호·유효기간·생년월일·사업자번호·카드 비밀번호·CVC는 결제대행사의 카드 등록·결제 요청에만 사용되고 회사 데이터베이스와 애플리케이션 로그에 저장하지 않습니다. 더페이원 카드 ID와 토스 빌링키 등 결제대행사가 발급한 결제수단 토큰 및 결제 연락처는 반복 결제와 결제 처리를 위해 AES-256-GCM으로 암호화해 저장하며 애플리케이션 로그에 기록하지 않습니다.</p>
      </LegalSection>
      ...
      <LegalSection title="5. 개인정보 처리업무의 위탁">
        ...
        <div className="overflow-x-auto"><table><thead><tr><th>수탁자</th><th>위탁 업무</th><th>처리될 수 있는 정보</th><th>주 처리 위치</th></tr></thead><tbody>
          ...
          <tr><td>토스페이먼츠 주식회사</td><td>카드 등록, 빌링키 발급, 구독 자동결제·취소 및 결제 결과 처리</td><td>토스 고객 식별자·인증키·빌링키, 카드 인증정보, 상품·금액·주문·거래 식별자</td><td>대한민국</td></tr>
        </tbody></table></div>
      </LegalSection>
