...
export function AdminShortsEventSetting({
  ...
  const activationBlocked = !environmentEnabled && !enabled;
    ...
    <section className="mt-5 rounded-2xl border border-white/10 bg-[#151819] p-5 sm:p-7">
      <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button
          ...
          className={`relative h-10 w-[72px] shrink-0 rounded-full border p-1 transition focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-white disabled:cursor-not-allowed disabled:opacity-40 ${
            enabled
      ...
      {!environmentEnabled ? (
        <p className="mt-5 rounded-xl border border-amber-300/20 bg-amber-300/[.06] px-4 py-3 text-xs leading-5 text-amber-100">
          배포 환경에서 종료된 이벤트로 고정되어 있습니다. 신규 안내와
          지급은 중단되며, 환경 설정을 명시적으로 켜기 전에는 관리자도
          DB 설정을 ON으로 변경할 수 없습니다.
        </p>
      ) : null}
      {message ? (
