...
export function AdminBillingDashboard({
  ...
  const remainingRefundable = refundOrder
    ...
    : 0;
  const terminationQuote = refundOrder
  ...
  const submitRefund = async () => {
    ...
    try {
      ...
      const result = await response.json() as { detail?: string; entitlementRequiresReview?: boolean; amountKrw?: number };
    ...
    <div className="mt-7 grid gap-7">
      ...
      {refundOrder && <div className="fixed inset-0 z-[100] grid place-items-center bg-black/75 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="refund-title">
        <div className="w-full max-w-lg rounded-3xl border border-white/10 bg-[#191c1d] p-6 shadow-2xl">
          ...
          <dl className="mt-5 grid grid-cols-[100px_1fr] gap-2 rounded-2xl bg-black/20 p-4 text-sm"><dt className="text-neutral-500">고객</dt><dd className="font-bold">{refundOrder.email}</dd><dt className="text-neutral-500">주문</dt><dd className="break-all font-mono text-xs">{refundOrder.orderId}</dd><dt className="text-neutral-500">잔여 가능액</dt><dd className="font-black">{money(remainingRefundable)}</dd><dt className="text-neutral-500">유료 필터</dt><dd className={refundOrder.popularFilterUsageCount > 0 ? "font-black text-amber-300" : "font-bold text-emerald-300"}>{refundOrder.popularFilterUsageCount > 0 ? `${refundOrder.popularFilterUsageCount}회 · 최근 ${date(refundOrder.popularFilterLastUsedAt)}` : "사용 이력 없음"}</dd></dl>
          ...
          {terminationQuote && <dl className="mt-4 grid grid-cols-[130px_1fr] gap-2 rounded-2xl border border-[#ff8c7c]/20 bg-[#ff8c7c]/[.06] p-4 text-sm">
            <dt className="text-neutral-400">경과 이용대금</dt><dd className="text-right font-bold">{money(terminationQuote.elapsedServiceKrw)}</dd>
            <dt className="text-neutral-400">잔여기간 이용대금</dt><dd className="text-right font-bold">{money(terminationQuote.remainingServiceKrw)}</dd>
            <dt className="text-neutral-400">중도해지 위약금</dt><dd className="text-right font-bold">{terminationQuote.withinSevenDays ? "7일 이내 면제" : money(terminationQuote.penaltyKrw)}</dd>
            <dt className="text-white">정책 환불액</dt><dd className="text-right font-black text-[#ffb4a8]">{money(terminationQuote.refundAmountKrw)}</dd>
          </dl>}
          ...
          <p className="mt-4 text-xs leading-5 text-amber-200/80">고객 귀책 중도해지는 서버가 환불 시점에 경과 이용대금과 잔여기간의 10%를 다시 계산합니다. 이 버튼은 더페이원 실제 환불 API를 호출하며, 구독 권한과 자동결제 상태는 관리자 확인 대상으로 남습니다.</p>
