...
const REVIEWED_DATE = "2026-07-26";
const priceFormatter = new Intl.NumberFormat("ko-KR");

export const metadata: Metadata = createPageMetadata({
  title: "AI 쇼츠 제작 툴 비교: 이지컷·알파컷·피카클립",
  description: "이지컷의 리뉴얼 요금제와 알파컷, 피카클립의 이용량 기준, AI 하이라이트, 자막, 템플릿과 편집 기능을 공식 공개정보 기준으로 정리합니다.",
  path: PAGE_PATH,
  type: "article",
});

function requirePricingPlan(code: PricingV2PlanCode) {
  const plan = getPricingV2Plan(code);
  if (!plan) throw new Error(`Missing pricing plan: ${code}`);
  return plan;
}

function packagePriceLines(tier: PricingV2PackageTier) {
  return pricingV2PackageMonths.map((months) => {
    const plan = getPricingV2Package(tier, months);
    if (!plan) throw new Error(`Missing pricing package: ${tier}_${months}m`);
    return `${months}개월 총 ${priceFormatter.format(plan.totalPriceKrw)}원 (월 환산 ${priceFormatter.format(plan.monthlyPriceKrw)}원)`;
  });
}

const easyCutProPlan = requirePricingPlan("easycut_pro_v2");
const easyCutEarlyBirdSummary = pricingV2EarlyBirdProducts
  .map((product) => `${priceFormatter.format(product.minutes)}분 ${priceFormatter.format(product.priceKrw)}원`)
  .join(" · ");
const easyCutPriceCards = [
  {
    title: easyCutProPlan.displayName,
    allowance: `월 ${priceFormatter.format(easyCutProPlan.monthlyMinutes)}분`,
    priceLines: [`월 ${priceFormatter.format(easyCutProPlan.monthlyPriceKrw)}원 · 매월 자동결제`],
    description: "필요한 기간만 월 단위로 시작할 수 있는 구독 상품입니다.",
  },
  {
    title: "스타터 패키지",
    allowance: "매월 200분",
    priceLines: packagePriceLines("starter"),
    description: "꾸준히 쇼츠를 제작하는 크리에이터를 위한 기간 패키지입니다.",
  },
  {
    title: "전문가 패키지",
    allowance: "매월 600분",
    priceLines: packagePriceLines("expert"),
    description: "제작량이 많은 크리에이터와 운영팀을 위한 기간 패키지입니다.",
  },
] as const;
...
const comparisonRows = [
  ...
  {
    label: "공개 플랜 제공량",
    easyCut: "월 60·200·600분",
    alphaCut: "월 50·150·450분",
    fikaClip: "월 5·15·50회",
  },
  {
    label: "유료 플랜 월간 결제",
    easyCut: "월 9,900원 또는 기간 패키지",
    alphaCut: "월 6,900·19,800·57,900원",
    fikaClip: "월 6,900·12,900·34,900원",
  },
  {
    label: "하이라이트 자동 추출",
    easyCut: "제공",
  ...
  },
  {
    ...
    easyCut: "4종",
    alphaCut: "커스텀 템플릿",
    fikaClip: "비디오·채널 스타일 템플릿",
  },
  {
    label: "프로젝트 보관",
    easyCut: "전 유료 상품 30일",
    alphaCut: "멤버십 편집 30일 안내",
    fikaClip: "공식 페이지에서 최신 정보 확인",
  },
  {
    label: "동시 작업",
    easyCut: "상품별 1·2·3개",
    alphaCut: "공식 페이지에서 최신 정보 확인",
    fikaClip: "공식 페이지에서 최신 정보 확인",
  },
] as const;
...
const selectionGuides = [
  {
    ...
    description: "영상 분량을 기준으로 월 작업량을 계산한다면 원본 처리시간을 제공량으로 표시하는 이지컷과 알파컷을 함께 살펴볼 수 있습니다.",
  },
  {
    title: "제작 횟수로 관리하고 싶다면",
    description: "매달 필요한 결과 수가 명확하다면 숏폼 제작 횟수를 기준으로 플랜을 안내하는 피카클립이 직관적일 수 있습니다.",
  },
  {
    title: "여러 작업을 동시에 돌린다면",
    description: "운영팀이라면 각 서비스의 동시 작업 한도와 프로젝트 보관기간을 공식 페이지에서 함께 확인하는 것이 좋습니다.",
  },
] as const;
    ...
    <div className="app-shell site-chrome min-h-screen text-neutral-100">
      ...
      <main className="mx-auto w-full max-w-6xl px-5 pb-24 pt-10 sm:px-8 sm:pt-14">
        <header className="mx-auto max-w-4xl py-12 text-center sm:py-16">
          ...
          <p className="mx-auto mt-6 max-w-3xl text-sm leading-7 text-neutral-400 sm:text-base">롱폼 영상을 짧은 쇼츠로 바꾸는 서비스마다 잘 맞는 제작 방식이 다릅니다. 이 페이지는 우열을 정하기보다 각 서비스의 공식 공개정보와 선택 기준을 같은 항목으로 정리합니다.</p>
          <p className="mt-4 text-xs text-neutral-500">공개정보 최종 확인일: 2026년 7월 26일</p>
        </header>

        <section aria-labelledby="quick-answer-title">
          <h2 id="quick-answer-title" className="text-2xl font-black tracking-[-.035em] text-white">서비스별로 잘 맞는 활용 방식</h2>
          <div className="mt-6 grid gap-4 md:grid-cols-3">
            <article className="rounded-2xl border border-[#ff8c7c]/30 bg-[#ff8c7c]/[.06] p-6">
              ...
              <p className="mt-3 text-sm leading-7 text-neutral-300">YouTube URL 중심의 간단한 흐름, 5가지 비율, 8개 제목 언어와 상품별 동시 작업 수가 필요한 경우 잘 맞습니다.</p>
            </article>
            <article id="alphacut" className="scroll-mt-28 rounded-2xl border border-white/10 bg-black/15 p-6">
              ...
              <p className="mt-3 text-sm leading-7 text-neutral-400">YouTube 링크와 파일 업로드, 무음 구간 제거·화자 트래킹 등 공식 안내 기능이 필요한 경우 잘 맞습니다.</p>
            </article>
            <article id="fikaclip" className="scroll-mt-28 rounded-2xl border border-white/10 bg-black/15 p-6">
              ...
              <p className="mt-3 text-sm leading-7 text-neutral-400">원본 분량보다 월 숏폼 제작 횟수 중심으로 플랜을 비교하고, 영상 업로드 정보까지 함께 확인하려는 경우 잘 맞습니다.</p>
            </article>
          </div>
        </section>

        <section aria-labelledby="renewed-pricing-title" className="mt-10">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              <p className="text-xs font-extrabold uppercase tracking-[.18em] text-[#ff9b8d]">Renewed pricing</p>
              <h2 id="renewed-pricing-title" className="mt-2 text-2xl font-black tracking-[-.035em] text-white">이지컷 리뉴얼 요금제</h2>
            </div>
            <Link href="/pricing" className="text-sm font-extrabold text-[#ff9b8d] hover:text-white">전체 요금제 보기 →</Link>
          </div>
          <p className="mt-4 max-w-3xl text-sm leading-7 text-neutral-400">월간 구독과 3·6·12개월 패키지 중 제작량과 이용기간에 맞춰 선택할 수 있습니다.</p>
          <div className="mt-6 grid gap-4 md:grid-cols-3">
            {easyCutPriceCards.map((card) => (
              <article key={card.title} className="rounded-2xl border border-[#ff8c7c]/20 bg-[#ff8c7c]/[.045] p-6">
                <div className="flex items-start justify-between gap-4">
                  <h3 className="font-black text-white">{card.title}</h3>
                  <span className="shrink-0 rounded-full bg-[#ff8c7c]/10 px-3 py-1 text-xs font-black text-[#ffb4a8]">{card.allowance}</span>
                </div>
                <ul className="mt-5 grid gap-2 text-sm leading-6 text-neutral-200">
                  {card.priceLines.map((line) => <li key={line}>{line}</li>)}
                </ul>
                <p className="mt-4 text-xs leading-6 text-neutral-500">{card.description}</p>
              </article>
            ))}
          </div>
          <p className="mt-4 text-xs leading-6 text-neutral-500">패키지는 선택한 기간의 총액을 결제하며, 표시된 월 금액은 비교를 위한 월 환산액입니다.</p>
          <p className="mt-2 text-xs leading-6 text-neutral-500">활성 이용권 보유자는 얼리버드 추가시간도 선택할 수 있습니다: {easyCutEarlyBirdSummary}. 각 상품은 계정당 한 번 구매할 수 있고 구매일로부터 90일간 유효합니다.</p>
        </section>
        ...
        <section aria-labelledby="comparison-table-title" className="mt-10">
          ...
          <p className="mt-4 text-xs leading-6 text-neutral-500">서비스별 상품과 기능은 수시로 바뀔 수 있습니다. 표에 구체적인 정보가 없는 항목은 미지원이라는 뜻이 아니며, 각 서비스의 공식 페이지에서 최신 내용을 확인해 주세요.</p>
        </section>

        <section aria-labelledby="alternative-title" className="mt-10">
          <h2 id="alternative-title" className="text-2xl font-black tracking-[-.035em] text-white">나에게 맞는 서비스를 고를 때 볼 것</h2>
          <div className="mt-6 grid gap-4 md:grid-cols-3">
        ...
        <section aria-labelledby="sources-title" className="mt-10">
          ...
          <p className="mt-6 text-xs leading-6 text-neutral-500">알파컷(AlphaCut)과 피카클립(FikaClip)은 각 소유자의 상표입니다. 이지컷은 두 서비스와 제휴하거나 공식적으로 연관되어 있지 않습니다. 각 서비스에는 서로 다른 강점과 적합한 사용 방식이 있으며, 본 비교는 공개정보를 이해하기 쉽게 정리한 참고 자료입니다. 특정 서비스의 우열이나 동일한 결과를 보장하지 않습니다.</p>
        </section>
