...
function ProjectWorkspace({ job, onBack, onChanged }: { job: VideoJob; onBack: () => void; onChanged: () => Promise<void> }) {
  ...
  return (
    <div className="project-workspace">
                ...
                <div className="short-result-layout">
                  <div className="short-video-column">
                    ...
                    <div className="short-result-actions">
                      <button disabled={job.isExample || itemIsRerendering} title={job.isExample ? "예시 작업은 편집할 수 없습니다." : undefined} onClick={() => { setSelectedId(item.id); setEditing(true); }} className="tool-button short-edit-button disabled:cursor-not-allowed disabled:opacity-40">✎ 편집하기</button>
      ...
      {editing && !job.isExample && <Editor item={selected} channelThumbnailUrl={job.channelThumbnailUrl} onClose={() => setEditing(false)} onChanged={onChanged} />}
    </div>
  );
}
