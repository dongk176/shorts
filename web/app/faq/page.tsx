...
const faqsByLocale: Record<SiteLocale, ReadonlyArray<readonly [string, string]>> = {
  ko: [
  ...
  en: [
    ["What does the Shorts AI do?", "It analyzes the audio and context of long videos, finds key segments, and automates the creation of short clips, hook titles, and caption drafts."],
  ...
  ja: [
    ["ショート動画AIは何をしますか？", "長尺動画の音声と文脈を分析して重要区間を見つけ、短い動画、フックタイトル、字幕案の作成を自動化します。"],
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return createPageMetadata({
    title: locale === "ko" ? "AI 쇼츠 제작 자주 묻는 질문 | 이지컷" : locale === "en" ? "AI Shorts FAQ | Easy Cut" : "AIショート動画作成 よくある質問 | Easy Cut",
    description: locale === "ko" ? "쇼츠 AI 기능, 생성 개수, 사용 가능한 YouTube 영상, 제목·자막 편집, 영상 길이 제한과 AI 쇼츠 툴의 차이를 확인하세요." : locale === "en" ? "Answers about Easy Cut's AI features, output count, supported videos, editing, length limits, and tool differences." : "Easy CutのAI機能、生成数、対応動画、編集、長さ制限、他ツールとの違いをご案内します。",
    path: PAGE_PATH,
  });
}
...
export default async function FaqPage() {
  ...
  const breadcrumbData = {
    ...
    "@type": "BreadcrumbList",
    itemListElement: [
      { "@type": "ListItem", position: 1, name: locale === "ko" ? "홈" : locale === "en" ? "Home" : "ホーム", item: SITE_URL },
      { "@type": "ListItem", position: 2, name: locale === "ko" ? "자주 묻는 질문" : locale === "en" ? "FAQ" : "よくある質問", item: `${SITE_URL}${PAGE_PATH}` },
    ],
  };
    ...
    <div className="app-shell site-chrome min-h-screen text-neutral-100">
      ...
      <main className="mx-auto w-full max-w-5xl px-5 pb-24 pt-10 sm:px-8 sm:pt-14">
        <header className="mx-auto max-w-3xl py-12 text-center sm:py-16">
          ...
          <h1 className="mt-5 text-[28px] font-black leading-tight tracking-[-.05em] text-white sm:text-[42px]">AI 쇼츠 제작<br /><span className="bg-gradient-to-r from-[#ff8c7c] to-violet-400 bg-clip-text text-transparent">자주 묻는 질문</span></h1>
