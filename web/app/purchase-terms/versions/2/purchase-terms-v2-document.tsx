...
import { purchaseTermsV7Translations, purchaseTermsV8Translations } from "@/lib/i18n/legal-translations";
...
export function PurchaseTermsV2Document({
  ...
  const supportsTossAutomaticBilling = version >= 8;
  return (
    <LegalDocument
      ...
      effectiveDate={archived
        ? `${version === 8 ? "2026년 8월 24일 · 구매약관 v8" : version === 7 ? "2026년 8월 18일 · 구매약관 v7" : version === 6 ? "2026년 8월 5일 · 구매약관 v6" : version === 5 ? "2026년 8월 3일 · 구매약관 v5" : version === 4 ? "2026년 7월 30일 · 구매약관 v4" : version === 3 ? "2026년 7월 28일 · 구매약관 v3" : "2026년 7월 26일 · 구매약관 v2"} (보관본)`
        : supportsTossAutomaticBilling
          ? "2026년 8월 24일 개정 · 구매약관 v8"
      ...
      translations={version === 8
        ? purchaseTermsV8Translations
        : version === 7 ? purchaseTermsV7Translations : undefined}
    >
      {archived && (
      ...
      <section aria-labelledby="purchase-summary" className="overflow-hidden rounded-3xl border border-[#ff8c7c]/25 bg-[linear-gradient(135deg,rgba(255,113,94,.12),rgba(160,120,255,.07))] p-5 sm:p-7">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          <div className="rounded-2xl border border-white/10 bg-black/15 p-4">
            <strong className="block text-sm text-white">{supportsTossAutomaticBilling ? "더페이원 월간 구독" : "월간 구독"}</strong>
            <p className="mt-2 text-xs leading-6 text-neutral-400">매월 같은 결제일에 자동결제되며, 해지 예약 전까지 갱신됩니다.</p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-black/15 p-4">
            <strong className="block text-sm text-white">{supportsTossAutomaticBilling ? "더페이원 기간 패키지" : "기간 패키지"}</strong>
          ...
          <div className="rounded-2xl border border-white/10 bg-black/15 p-4">
            <strong className="block text-sm text-white">{supportsTossAutomaticBilling ? "토스 자동결제" : "추가 처리시간"}</strong>
            <p className="mt-2 text-xs leading-6 text-neutral-400">{supportsTossAutomaticBilling ? "1·6·12개월 계약 총액을 결제하고 계약 종료일마다 같은 기간으로 자동갱신합니다." : "일반결제로 구매하며 승인일부터 90일 동안 사용할 수 있습니다."}</p>
          </div>
        </div>
      </section>
      ...
      {supportsTossAutomaticBilling ? (
        <LegalSection title="제11조 결제수단별 적용 조건 및 토스 자동결제">
          <p>기존 더페이원 결제자와 결제 확인 화면에서 더페이원 결제로 안내된 주문에는 제3조부터 제6조까지의 월간 구독·기간 패키지 조건을 계속 적용합니다. 아래 토스 자동결제 조건은 결제 확인 화면에서 토스 자동결제로 안내된 이용자와 주문에만 적용하며, 서로 다른 결제대행사의 주문·카드·자동결제 일정을 섞거나 임의로 이전하지 않습니다.</p>
          <p>토스 자동결제의 이지컷 프로·스타터·전문가 플랜은 1·6·12개월 계약 상품입니다. 최초 구독과 각 계약 갱신일에 선택한 계약기간의 총액을 결제하며, 요금제 화면의 월 환산 금액은 비교를 위한 표시일 뿐 매월 나누어 청구되는 금액이 아닙니다. 처리시간과 이용 한도는 계약이 활성인 동안 매월 기준일에 해당 플랜의 월 제공량으로 지급됩니다.</p>
          <p>이용자가 해지를 예약하면 이미 결제한 계약기간과 지급된 권한은 종료일까지 유지되고 다음 계약 자동갱신만 중단됩니다. 현재 플랜보다 계약 총액이 높은 플랜으로 변경하면 현재 계약의 남은 시간 비율에 따른 미사용 금액을 공제한 차액을 승인한 뒤 즉시 전환합니다. 계약 총액이 같거나 낮은 플랜은 현재 계약 종료일로 변경을 예약하며, 예약 시에는 결제하지 않고 종료일에 새 플랜 총액 승인이 완료된 뒤 전환합니다.</p>
          <p>토스 자동결제는 토스페이먼츠 결제창에서 카드 등록과 본인확인을 거쳐 발급된 빌링키로 승인합니다. 회사는 빌링키를 다시 암호화해 저장하고 카드사 코드·마스킹 카드번호·끝 4자리 등 최소 정보만 보관하며, 원 카드번호·유효기간·생년월일·사업자번호·카드 비밀번호·CVC를 저장하지 않습니다. 카드사 심사 등으로 이용할 수 없는 카드는 결제 전 화면에 안내하고, 제한 상태에서 발급된 빌링키는 저장하거나 청구하지 않습니다.</p>
        </LegalSection>
      ) : null}
    </LegalDocument>
  );
