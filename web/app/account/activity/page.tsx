...
export default async function AccountActivityPage() {
  ...
  try {
    session = await requireAuthenticatedMvpSession();
  } catch (error) {
  ...
  return (
    <div className="app-shell site-chrome min-h-screen text-neutral-100">
      <SiteHeader><AuthControls user={session.user} next="/account/activity" /></SiteHeader>
      <main className="mx-auto w-full max-w-[1440px] px-5 py-12 sm:px-8">
