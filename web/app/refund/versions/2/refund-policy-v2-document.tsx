...
export function RefundPolicyV2Document({
  archived = true,
  version = 2,
}: {
  archived?: boolean;
  ...
  return (
    <LegalDocument
      ...
      effectiveDate={archived
        ? "2026년 7월 26일 · 환불정책 v2 (보관본)"
        : usesFirstCompletedJobPolicy
          ? "2026년 7월 28일 개정 · 환불정책 v3"
          : "2026년 7월 26일 개정 · 환불정책 v2"}
    ...
    >
      {archived && (
        <p className="rounded-2xl border border-white/10 bg-white/[.035] p-4 text-sm leading-7 text-neutral-300">
          이 페이지는 환불정책 v2 보관본입니다. 현재 정책은{" "}
          <Link href="/refund" className={accentLink}>취소 및 환불 정책</Link>에서 확인할 수 있습니다.
        </p>
      )}
      <section aria-labelledby="refund-summary" className="overflow-hidden rounded-3xl border border-[#ff8c7c]/25 bg-[linear-gradient(135deg,rgba(255,113,94,.12),rgba(160,120,255,.07))] p-5 sm:p-7">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          ...
          <div className="rounded-2xl border border-white/10 bg-black/15 p-4">
            <strong className="block text-sm text-white">첫 작업 기준 환불</strong>
            <p className="mt-2 text-xs leading-6 text-neutral-400">{usesFirstCompletedJobPolicy ? "첫 작업이 완료됐다면 실 결제금액의 1개월분만 공제하고, 완료되지 않았다면 1개월분을 공제하지 않습니다." : "완료된 월과 현재 사용한 월의 계약 월단가를 제외하고 아직 제공되지 않은 미래 월 이용료를 환불합니다."}</p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-black/15 p-4">
      ...
      <LegalSection title="제7조 구독·기간 패키지 사용 후 환불">
        <p>{usesFirstCompletedJobPolicy ? "환불정책 v3가 적용되는 구독과 기간 패키지는 해당 주문에서 공급한 이용권으로 첫 유료 작업이 정상 완료되었는지를 기준으로 환불액을 계산합니다." : "환불정책 v2가 적용된 기간 패키지는 완료된 월과 현재 사용한 월의 이용대금을 제외하고 현재 미사용 월 및 아직 개시되지 않은 미래 월의 이용대금을 환불합니다."}</p>
        <div className="rounded-2xl border border-white/10 bg-white/[.035] p-5 text-center sm:p-6">
          <p className="text-xs font-bold tracking-wide text-neutral-500">{usesFirstCompletedJobPolicy ? "첫 작업 완료 기준 환불액" : "기간 패키지 환불액"}</p>
          <p className="mt-2 text-base font-black leading-7 text-white sm:text-lg">{usesFirstCompletedJobPolicy ? "실 결제금액 − 첫 작업 완료 시 1개월분 − 기존 환불액" : "실 결제금액 − 제공 완료 월 이용대금 − 기존 환불액"}</p>
        </div>
        {usesFirstCompletedJobPolicy ? <ul>
          <li>• 첫 작업 완료 전에는 1개월분을 공제하지 않습니다.</li>
          <li>• 첫 작업이 정상 완료되면 실 결제금액을 계약 개월 수로 나눈 1개월분만 공제합니다.</li>
          <li>• 경과 일수, 이미 지나간 달 수, 잔여기간 10% 위약금, 결제대행 수수료 또는 별도 환불 처리비는 중복 공제하지 않습니다.</li>
          <li>• 첫 작업 완료 여부는 해당 주문과 연결된 처리시간 원장 및 완료 작업 기록으로 확인합니다.</li>
          <li>• 첫 작업 완료로 1개월분을 공제한 경우 현재 이용 월 종료일까지 권한을 유지하도록 처리할 수 있으며, 즉시 종료를 선택하면 남은 권한과 기본 처리시간을 회수합니다.</li>
