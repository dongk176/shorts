...
Worker의 YouTube JavaScript 실행 환경은 `yt-dlp==2026.8.19`, `yt-dlp-ejs==0.8.0`, Deno `2.8.3` 조합으로 고정합니다. 호환성 문제를 피하려면 세 버전을 함께 검토하고 갱신해야 합니다. EJS는 일반적인 YouTube player JavaScript 처리를 지원하지만 다운로드 성공을 보장하지 않습니다. 계정 쿠키는 사용하지 않으며, Proof-of-Origin은 공개 영상에 한해 기능 플래그가 켜진 고정 provider에서만 사용합니다.
