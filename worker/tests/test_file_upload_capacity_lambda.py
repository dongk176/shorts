...
def test_twenty_first_session_waits_without_receiving_a_duplicate_grant(
    monkeypatch,
) -> None:
    module, _table, ecs, elbv2 = load_capacity_lambda(monkeypatch)
    monkeypatch.setattr(module.time, "time", lambda: 1_000)
    ...
    assert state["desiredCount"] == 20
    assert state["grantedCount"] == 20
    assert state["waitingCount"] == 1
    assert state["leaseState"] == "waiting"
