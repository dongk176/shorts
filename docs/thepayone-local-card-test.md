...
카드 등록이 완료되면 같은 로컬 화면에서 등록된 `cardId`로 실제 결제 테스트를 시작할 수 있습니다.

- 서버 고정 금액 1,000원을 최대 3회 승인합니다.
- 첫 승인은 테스트 시작 버튼을 누른 즉시 실행하고, 성공한 시점부터 1분 간격으로 2회 더 실행합니다.
- 등록된 `cardId` 승인은 통합매뉴얼 3.2 기준으로 `trxType="ONTR"`, `udf2="00"`, `products` 배열, `metadata.recurring="pay"`를 사용합니다. 구인증을 활성화하는 `cardAuth`와 카드 비밀번호는 재전송하거나 저장하지 않습니다.
- 브라우저가 열린 동안 1초 단위로 일정을 표시하고, 10초마다 DB 상태를 동기화합니다.
- 일정과 회차는 DB에 저장되므로 새로고침이나 개발 서버 재시작 후 화면을 다시 열면 이어집니다.
- 사용자가 중단하면 아직 실행되지 않은 다음 회차만 중지되며, 이미 승인된 결제는 자동 취소되지 않습니다.
- 네트워크 단절처럼 승인 결과가 불명확하면 자동 재시도하지 않고 `unknown`으로 중단합니다. PG 관리자 화면에서 승인 여부를 확인해야 합니다.
- 이 테스트는 `user_subscriptions`나 사용자 플랜을 변경하지 않습니다.

하나카드는 현재 더페이원 정기결제 등록을 지원하지 않습니다. 카드사 선택 단계에서 하나카드를 차단하고 안내 오버레이를 표시하며, 서버에서도 동일하게 거부합니다. 공급자 오류 메시지에 하나카드가 식별되는 경우에도 같은 안내로 변환합니다.
