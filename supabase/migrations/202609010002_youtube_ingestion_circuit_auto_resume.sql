...
-- Keep the live route-quality controls reproducible.  These additions are
-- insert-only/additive and do not rewrite existing attempts or route leases.
...
-- The original claim body is kept separately so the small circuit wrapper can
-- auto-clear only known transient bot-check states without duplicating work.
create or replace function shorts_mvp.claim_project_job_outbox_without_ingestion_circuit(
...
create or replace function shorts_mvp.claim_project_job_outbox(
  p_limit integer default 100
)
returns table (
  outbox_id uuid,
...
begin
  select reason,blocked_until into circuit_reason,circuit_blocked_until
...
create or replace function shorts_mvp.update_ingestion_circuit_from_attempt()
returns trigger
language plpgsql
security definer
set search_path = shorts_mvp, pg_temp
...
begin
  if new.result='success' then
    ...
    set blocked_until=null,
        ...
        updated_at=clock_timestamp()
    where singleton
      and reason in ('bot_check_active','bot_check_rate');
  end if;
  return new;
end;
$$;

drop trigger if exists ingestion_attempts_update_circuit
  on shorts_mvp.ingestion_attempts;
create trigger ingestion_attempts_update_circuit
after insert on shorts_mvp.ingestion_attempts
for each row execute function shorts_mvp.update_ingestion_circuit_from_attempt();
...
revoke all on function shorts_mvp.rotate_ingestion_route(uuid,text,text,integer,text[])
  from public, anon, authenticated;
revoke all on function shorts_mvp.update_ingestion_circuit_from_attempt()
  from public, anon, authenticated;
