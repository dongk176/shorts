...
alter table shorts_mvp.video_jobs
  ...
  add column if not exists selected_source_duration_seconds numeric(12,3),
  add column if not exists billable_source_seconds integer;
...
begin
  if not exists (
    ...
    where conrelid='shorts_mvp.youtube_analyses'::regclass
      and conname='youtube_analyses_positive_duration_v2_check'
  ) then
    alter table shorts_mvp.youtube_analyses
      add constraint youtube_analyses_positive_duration_v2_check
...
begin
  if not exists (
    ...
    where conrelid='shorts_mvp.video_jobs'::regclass
      and conname='video_jobs_positive_source_duration_v2_check'
  ) then
    alter table shorts_mvp.video_jobs
      add constraint video_jobs_positive_source_duration_v2_check
      check (source_duration_seconds > 0) not valid;
  end if;
  if not exists (
    select 1 from pg_constraint
    where conrelid='shorts_mvp.video_jobs'::regclass
      and conname='video_jobs_source_range_selection_v2_check'
  ) then
    alter table shorts_mvp.video_jobs
      add constraint video_jobs_source_range_selection_v2_check check (
        ...
        or (
          ...
          and (
            selected_source_duration_seconds is null
            or selected_source_duration_seconds = range_end_seconds - range_start_seconds
          )
          and (
            billable_source_seconds is null
            or (
              billable_source_seconds between 60 and 3600
              and billable_source_seconds % 60 = 0
            )
          )
        )
      ) not valid;
...
insert into shorts_mvp.runtime_feature_flags (flag_key,enabled,description)
values (
  'source_range_selection_public',
  false,
  '구간 선택을 어드민 카나리에서 전체 사용자로 승격하는 공개 스위치'
)
on conflict (flag_key) do nothing;
...
comment on column shorts_mvp.video_jobs.selected_source_duration_seconds is
  'Exact selected source range length before billing rounding';
comment on column shorts_mvp.video_jobs.billable_source_seconds is
  'Usage seconds reserved for the selected source range';
