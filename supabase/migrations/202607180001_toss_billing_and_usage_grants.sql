...
alter table shorts_mvp.plans
  add constraint plans_code_check check (code in ('free','plus','standard','pro'));
alter table shorts_mvp.plans
...
create unique index if not exists app_users_toss_customer_key_idx
  on shorts_mvp.app_users (toss_customer_key)
...
create table if not exists shorts_mvp.billing_webhook_events (
...
begin
  foreach table_name in array array[
    ...
    'usage_grants','usage_grant_allocations','billing_webhook_events'
  ] loop
